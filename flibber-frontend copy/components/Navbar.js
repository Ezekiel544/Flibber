...
export default function Navbar({ account, onConnect, onDisconnect, chainId, connecting }) {
  ...
  const links = [
    ...
    // Dashboard only shows for admin wallet
    ...(isAdmin ? [{ href: '/dashboard', label: '⚙️ Admin' }] : []),
  ]
      ...
      <div style={{ position: 'relative', flexShrink: 0 }}>
        ...
        {showMenu && account && (
          <div style={{
            ...
            {isAdmin && (
              <div style={{ padding: '4px 12px', fontSize: '11px', color: '#00FF87', fontWeight: '700' }}>
                ⚙️ Admin
              </div>
            )}
            ...
            <Link href="/history" onClick={() => setShowMenu(false)}
              style={{ display: 'block', padding: '8px 12px', fontSize: '13px', color: '#E8F0F8', textDecoration: 'none', borderRadius: '6px' }}>
              Slot History
            </Link>
            {isAdmin && (
              <Link href="/dashboard" onClick={() => setShowMenu(false)}
                style={{ display: 'block', padding: '8px 12px', fontSize: '13px', color: '#00FF87', textDecoration: 'none', borderRadius: '6px' }}>
                Admin Dashboard
              </Link>
            )}
